...
function boot() {
  ...
  function hit(enemy, damage) {
    ...
    if (enemy.hp <= 0) {
      ...
      for (let i = 0; i < gems; i += 1) {
        game.gems.push({ x: enemy.x + random(-14, 14), y: enemy.y + random(-14, 14), value: enemy.value });
      }
  ...
  function shoot() {
    ...
    if (w.nova.level > 0 && game.t - game.lastNova >= w.nova.rate * hasteCooldown) {
      ...
      for (const target of game.enemies) {
        if (target.hp > 0 && distance(target, p) <= radius + target.r) hit(target, weaponDamage(w.nova));
      }
  ...
  function drawGrid(p) {
    ...
    ctx.strokeStyle = settings.colorMode === 'contrast' ? '#ffffff33' : '#78d2ff14';
    ctx.lineWidth = 1;
    const step = 70;
    ...
    if (settings.graphics !== 'Low') {
      for (let radius = 180; radius < 900; radius += 180) {
        ctx.beginPath();
  ...
  function drawPlayer(cx, cy) {
    ...
    ctx.save();
    ctx.translate(cx, cy);
    glow(30, skinImg ? '#67e8f9' : skin.glow);
    if (skinImg) {
      ctx.save();
      ctx.beginPath();
  ...
  function draw() {
    ...
    gradient.addColorStop(0, settings.colorMode === 'contrast' ? '#244e78' : '#193c61');
    ...
    for (const enemy of game.enemies) {
      ...
      glow(enemy.kind === 'boss' ? 30 : 14, enemy.color);
      ctx.beginPath();
      if (['tank', 'elite', 'boss'].includes(enemy.kind)) {
      ...
      if (enemy.hp < enemy.max && enemy.kind !== 'boss') {
